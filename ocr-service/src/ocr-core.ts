// Unchanged from server/ocr-service.ts in the main app — this whole feature
// depends on pdf2pic shelling out to GraphicsMagick/Ghostscript, which Vercel
// serverless functions cannot run. This service runs on a normal persistent
// Node host (Render/Fly/Railway) where those binaries can be installed, so
// its local-disk temp-file usage (unlike the main app) is fine as-is.
...
async function convertPdfToImages(pdfPath: string): Promise<string[]> {
  const outputDir = path.join(os.tmpdir(), "ocr-temp");
...
export async function extractCaregiverFromPdf(
  ...
  try {
    ...
    return await extractDataFromImages<ExtractedCaregiverData>(
      imagePaths,
...
export async function extractClientFromPdf(
  ...
  try {
    ...
    return await extractDataFromImages<ExtractedClientData>(
      imagePaths,
...
export async function convertPdfToAllPageImages(pdfPath: string): Promise<PageImageInfo[]> {
  const outputDir = path.join(os.tmpdir(), "paystub-pages");
