...
<body style="margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f0f4f8;">
  ...
  app.post("/api/exclusions/upload/sam", isAuthenticated, csvUpload.single('file'), async (req: any, res) => {
    try {
      const user = req.session?.user;
      if (!user || (user.role !== "admin" && user.role !== "supervisor" && user.role !== "super_admin")) {
        return res.status(403).json({ message: "Unauthorized: Admin role required" });
      ...
      const csvContent = req.file.buffer.toString('utf-8');
      const { exclusionService, SamImportValidationError } = await import('./exclusion-service');
      try {
        ...
        if (importErr instanceof SamImportValidationError) {
          return res.status(importErr.statusCode).json({
            success: false,
            recordCount: 0,
            errors: [importErr.message],
  ...
  app.post("/api/doh-audits", isAuthenticated, async (req: any, res) => {
    ...
    try {
      const audit = await storage.createDohAuditAssessment({
        ...
        surveyPeriod: surveyPeriod ? String(surveyPeriod) : null,
        surveyorName: surveyorName ? String(surveyorName) : null,
        auditDate: auditDate ? String(auditDate) : null,
        createdBy: user.id,
        status: "in_progress",
      });
  ...
  // Routes extracted to ./audit-saved-comparison-routes.ts so they can be
  // mounted onto a fresh Express app in unit tests.
  registerDohSavedComparisonRoutes(app, isAuthenticated);
